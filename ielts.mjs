...
export async function showIelts(mount, onScore, onBack) {
  ...
  mount.onclick = async event => {
    ...
    if(button.hasAttribute('data-ielts-back')){mount.onclick=null;onBack();return;}
    if(button.hasAttribute('data-ielts-categories')){category=null;selected=null;render();return;}
    if(button.dataset.ieltsCategory || button.hasAttribute('data-ielts-next')){if(button.dataset.ieltsCategory)category=button.dataset.ieltsCategory;mount.innerHTML='<p>Loading exercise…</p>';try{const response=await fetch(`/api/ielts/exercise?category=${category}`);if(!response.ok)throw Error('Exercise unavailable');selected=await response.json();answers={};writing='';submitted=false;result=null;render();}catch{mount.innerHTML='<div class="notice">IELTS exercises are unavailable on this server.</div><button class="btn secondary" data-ielts-categories>Back</button>';}return;}
    if(button.hasAttribute('data-ielts-submit')){try{const response=await fetch('/api/ielts/submit',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({category,id:selected.id,answers})});if(!response.ok)throw Error('Could not submit exercise');result=await response.json();submitted=true;await onScore({mode:`ielts-${category}`,correct:result.correct,total:result.total,words:writing.trim()?writing.trim().split(/\s+/).length:0});render();}catch{alert('Could not submit this exercise. Try again.');}}
  };
