...
async function profileById(id) {const rows=await sql`select id,name,password_salt,password_hash,claim_code_hash from studio.profiles where id=${id} limit 1`;return rows[0]||null;}
async function sessionFor(request) {
  ...
  const rows=await sql`select p.id,p.name,s.token_hash from studio.sessions s join studio.profiles p on p.id=s.profile_id where s.token_hash=${hash} and s.expires_at>now() limit 1`;
  return rows[0]||null;
}
async function issueSession(profile) {
  ...
  return {token,user:{id:profile.id,name:profile.name}};
}
...
Deno.serve(async request => {
  ...
  try {
    ...
    if(path==='/claim'&&request.method==='POST') {
      ...
      const changed=await sql`update studio.profiles set password_salt=${salt},password_hash=${hash},claim_code_hash=null where id=${id} and password_hash is null and claim_code_hash=${profile.claim_code_hash} returning id,name`;
