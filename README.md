...
The [public app](https://datawithmursalin-cloud.github.io/GRE-IELTS-Practice-Studio/) is hosted on GitHub Pages, with profile authentication and score history handled by the Supabase `studio-api` Edge Function and a private database schema. Syed (formerly Mursalin) and Ramisa are built-in profiles; new participants can create a name and password. Passwords must be at least 4 characters and are stored only as salted PBKDF2 hashes; longer, unique passwords are safer. Account settings let a signed-in user rename their profile, change their password (signing out other devices), or permanently delete the profile and its synced score history after entering the current password. There is no email-based password recovery; an administrator must reset a forgotten password.
