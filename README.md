...
The [public app](https://datawithmursalin-cloud.github.io/GRE-IELTS-Practice-Studio/) is hosted on GitHub Pages, with profile authentication and score history handled by the Supabase `studio-api` Edge Function and a private database schema. Mursalin and Ramisa are built-in profiles; each uses a one-time setup code to choose a password on first sign-in. New participants can create a name and password. Passwords must be at least 4 characters and are stored only as salted PBKDF2 hashes; longer, unique passwords are safer. Signed-in users can change their password, which signs out other devices. There is no email-based password recovery; an administrator must reset a forgotten password.
