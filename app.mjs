...
document.addEventListener('submit',async event=>{
  ...
  event.preventDefault();
  try{
    const payload=event.target.id==='select-profile-form'?{id:document.querySelector('#profile-select').value,...(pagesMode?{password:document.querySelector('#profile-password').value}:{})}:{name:document.querySelector('#profile-name').value,...(pagesMode?{password:document.querySelector('#new-profile-password').value}:{})};
